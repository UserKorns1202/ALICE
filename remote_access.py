...
class _RemoteHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path == '/client':
            ...
            html = (
                ...
                "function executeAction(act){try{if(!act||!act.action)return;const a=act.action; if(a==='open_url' && act.url){log('Executing: open '+act.url,'evt');try{window.open(act.url,'_blank');}catch(e){log('open_url failed: '+e,'err');}}else if(a==='play_audio' && act.url){log('Executing: play '+act.url,'evt');try{const audio=new Audio(act.url);audio.play().catch(e=>log('play failed: '+e,'err'));}catch(e){log('play_audio failed: '+e,'err');}}else if(a==='notify' && act.title){try{notify(act.title, act.body||'');}catch(e){}}else{log('Unknown exec action: '+JSON.stringify(act),'err');}}catch(e){log('executeAction exception: '+e,'err');}}"

                "function sendCommand(text){try{const trimmed=text.trim();let payload={text:text,token:token};const lower=trimmed.toLowerCase();let exec=null; // decide if should execute on sender\n                    // Heuristics: 'open <url>' or 'play <url>'\n                    if(lower.startsWith('open ')||lower.startsWith('play ')){\n                        const parts=trimmed.split(/\\s+/,2);\n                        if(parts.length===2 && parts[1].match(/https?:\\/\\//)){\n                            const url=parts[1];\n                            payload.execute_on_sender=true;\n                            payload.exec_action = lower.startsWith('open ') ? 'open_url' : 'play_audio';\n                            payload.exec_url = url;\n                        }\n                    }\n                    fetchWithAuth('/voice-command',{method:'POST',body:JSON.stringify(payload)}).then(r=>r.json()).then(j=>{if(j.ok){if(j.execute){log('Execute instruction received','voice');executeAction(j.execute);}if(j.queued){log('Voice command sent: '+(j.command||'[queued]'),'voice');notify('ALICE','Command: '+(j.command||'[queued]'));if(j.session_id&&audioEnabled){checkForAudioResponse(j.session_id);}}else if(!j.execute){log('Voice command ignored: '+(j.message||'no command'),'evt');}}else{log('Voice send failed: '+JSON.stringify(j),'err');}}).catch(e=>log('Voice send error: '+e,'err'));}catch(e){log('sendCommand exception: '+e,'err');}}"
    ...
    def do_POST(self):
        ...
        if p.path == '/voice-command':
            ...
            if command_text:
                # If requester asked that the command execute on the sending device,
                # return an execute instruction immediately instead of queueing to ALICE.
                if data.get('execute_on_sender') or data.get('exec_action'):
                    exec_action = data.get('exec_action') or data.get('exec_action')
                    exec_url = data.get('exec_url') or data.get('exec_url')
                    # If client sent exec fields, use them; else try to infer from text
                    if not exec_action and command_text.lower().startswith('open '):
                        exec_action = 'open_url'
                        exec_url = command_text.split(None,1)[1].strip() if ' ' in command_text else ''
                    if not exec_action and command_text.lower().startswith('play '):
                        exec_action = 'play_audio'
                        exec_url = command_text.split(None,1)[1].strip() if ' ' in command_text else ''
                    exec_payload = None
                    if exec_action:
                        exec_payload = {'action': exec_action}
                        if exec_url:
                            exec_payload['url'] = exec_url
                        add_event('execute', session_id=session_id, execute=exec_payload)
                        return self._ok({'queued': False, 'execute': exec_payload, 'session_id': session_id})

                # Track this as a remote session for audio response (normal path)
