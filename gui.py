...
class OverlayWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        # Start click-through and translucent by default
        ...
        # Hover check timer: poll global cursor position so we can react to hover
        # even when the window is click-through (WA_TransparentForMouseEvents).
        self._is_hovered = False
        self._hover_check_timer = QtCore.QTimer()
        self._hover_check_timer.timeout.connect(self._check_hover)
        self._hover_check_timer.start(100)  # 100ms polling

        # Start solid; on hover become transparent
    ...
    def enterEvent(self, event: QtGui.QEnterEvent) -> None:
        ...
        try:
            # Prefer event-based handling when available
    ...
    def _check_hover(self) -> None:
        """Poll the global cursor position and toggle opacity when hovering.

        This allows hover-based opacity changes even when the window is set
        to be click-through and therefore does not receive enter/leave events.
        """
        try:
            pos = QtGui.QCursor.pos()
            # geometry() is in global coordinates
            inside = self.geometry().contains(pos)
            if inside and not self._is_hovered:
                self._is_hovered = True
                try:
                    self.setWindowOpacity(self._hover_opacity)
                except Exception:
                    pass
            elif not inside and self._is_hovered:
                self._is_hovered = False
                try:
                    self.setWindowOpacity(self._normal_opacity)
                except Exception:
            ...
            # ignore any errors from querying cursor or geometry
            pass
