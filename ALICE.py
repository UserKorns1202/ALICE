...
# Global context awareness instance (used to provide subtle tone hints to the LLM)
try:
    context_agent = context_awareness.ContextAwareness()
except Exception:
    context_agent = None
...
# Move speaking/listening logic into a separate module for easier editing.
# The `speech_io` module implements a compatible subset and will copy
# its public names into this module's globals so existing code can keep
# referencing `speak`, `listen`, `speak_queue`, etc.
try:
    import speech_io
    speech_io.integrate(globals())
except Exception as _e:
    print(f"[Speech] Failed to integrate speech_io: {_e}")
...
# Conversation history managed via ConversationManager. `conversation_history`
# remains a list reference (the manager.history list) for compatibility with
# other modules that append or slice it directly.
...
def save_conversation_history():
    try:
        conversation_manager.save("conversation_history.json")
    except Exception:
        # Fallback: write the raw list
        try:
...
def load_conversation_history():
    ...
    try:
        conversation_manager.load("conversation_history.json")
        conversation_history = conversation_manager.history
...
def _tts_generator_loop():
    ...
    while not is_exiting:
        try:
            ...
            try:
                try:
                    wav = piper_tts.speak(text, voice=current_tts_voice, play=False)
                except TypeError:
                    wav = piper_tts.speak(text, play=False)
            except Exception as e:
        ...
        try:
            ...
            try:
                if wav and os.path.exists(wav):
                    ...
                    try:
                        piper_tts.speak(text, voice=current_tts_voice, play=True, block=True)
                    except TypeError:
                        try:
                            piper_tts.speak(text, play=True, block=True)
                        except Exception as e:
...
def listen_for_interrupt():
    ...
    while interrupt_thread_running.is_set():
        try:
            # Avoid competing microphone access by acquiring the global listen_lock
            with listen_lock:
...
def listen():
    recognizer = sr.Recognizer()
    try:
        # Prevent concurrent microphone usage
        with listen_lock:
            with sr.Microphone() as source:
                print("Listening...")
                send_listening_command()
                ...
                try:
                    ...
                    print(f"[listen] Microphone/listen error: {e}")
                    return None

    except Exception as e:
        print(f"[listen] Microphone setup error: {e}")
        return None
...
def start_gui():
    ...
    try:
        ...
        guiProcess = subprocess.Popen(["python", "gui.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print("[GUI] GUI process started successfully")
    except Exception as e:
...
def toggle_input_mode():
    global input_mode, listening_active, is_speaking
    old_mode = (input_mode or "").strip().lower()
    # Normalize and toggle
    input_mode = "typing" if old_mode == "speaking" else "speaking"
    print(f"[Mode] {old_mode} -> {input_mode}")

    # Update GUI and runtime flags
    try:
        if input_mode == "speaking":
            # Enter speaking/listening mode
            listening_active = True
            try:
                send_listening_command()
            except Exception:
            ...
            # Switch to typing: stop listening/speaking and notify GUI
            listening_active = False
            try:
                send_idle_command()
            except Exception:
                pass
            # Attempt to stop any ongoing TTS/playback and interrupt threads
            try:
                stop_interrupt_thread()
            except Exception:
                pass
            try:
                force_stop_speech_engine()
            except Exception:
                pass
            # Ensure speaking flag cleared
            try:
                is_speaking = False
            except Exception:
                pass
            # Optionally give a short confirmation via print (avoid speaking to prevent auto TTS)
            print("Switched to typing mode")
    except Exception as e:
        print(f"[Mode] toggle helper error: {e}")

# Function to retrieve current mode for other programs
def get_current_input_mode():
    global input_mode
    try:
        return (input_mode or "").strip().lower()
    except Exception:
        return "typing"
...
def resolve_shortcut(shortcut_path):
    ...
    try:
        ...
        if os.path.exists(shortcut_path):
            try:
                # Use the com_init context manager to ensure COM is initialized
                ...
                # Best-effort fallback (may still work without explicit COM init)
                try:
                    from com_utils import com_init
                    import win32com.client
                    with com_init():
...
def verify_and_confirm(program_name, path):
    """Enhanced verification with timeout and child process check."""
    ...
    try:
        ...
        for proc in psutil.process_iter(['name', 'exe']):
            ...
            elif proc.info['name'] and program_name.lower() in proc.info['name'].lower():
                return f"Successfully opened {program_name}."
    except ImportError:
        pass
    return f"Opened {program_name}, but verification uncertain."
        ...
        if platform.system() == "Windows":
            ...
            if closest_match.endswith('.lnk'):
                ...
                if target_path:
                    ...
                    try:
                        os.startfile(target_path)  # Safer than shell
                        return verify_and_confirm(program_name, target_path)
                    except Exception as e:
                        return f"Failed to open shortcut: {e}"
                else:
                ...
                if os.path.exists(full_path):
                    try:
                        os.startfile(full_path)  # Safer than shell
                        return verify_and_confirm(program_name, full_path)
                    except Exception as e:
                ...
                try:
                    subprocess.run(["start", closest_match], shell=False, timeout=10)
                    return verify_and_confirm(program_name, closest_match)
                except subprocess.TimeoutExpired:
                    return f"Timeout opening {program_name}."
                except Exception as e:
                ...
                try:
                    subprocess.run([executable_path], timeout=10)
                    return verify_and_confirm(program_name, executable_path)
                except subprocess.TimeoutExpired:
                    return f"Timeout opening {program_name}."
                except Exception as e:
            ...
            try:
                subprocess.run([closest_match], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10)
                return verify_and_confirm(program_name, closest_match)
            except subprocess.TimeoutExpired:
                return f"Timeout opening {program_name}."
            except Exception as e:
                return f"Failed to open on Linux: {e}"

        elif platform.system() == "Darwin":  # macOS
            try:
                subprocess.run(["open", "-a", closest_match], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10)
                return verify_and_confirm(program_name, closest_match)
            except subprocess.TimeoutExpired:
                return f"Timeout opening {program_name}."
            except Exception as e:
                return f"Failed to open on macOS: {e}"
...
def find_closest_process(program_name, threshold=0.6):
    ...
    try:
        ...
        return None
    low = (program_name or "").strip().lower()
    if not low:
        return None

    # Build a list of running processes with name/exe
    procs = []
    try:
        ...
        # Fallback: try simpler iteration
        try:
            for proc in psutil.process_iter(['name']):
                name = proc.info.get('name') or ''
                if name:
                    procs.append({'pid': proc.pid, 'name': name, 'exe': ''})
        except Exception:
            return None

    # Avoid killing core/system processes
    blacklist = set(['system', 'idle', 'services.exe', 'svchost.exe', 'wininit.exe', 'lsass.exe', 'csrss.exe'])

    # 1) Prefer direct substring matches in name or executable basename
    substring_matches = []
    for p in procs:
        n = (p['name'] or '').lower()
        e = os.path.basename(p['exe']).lower() if p['exe'] else ''
        if low in n or (e and low in e):
            if n not in blacklist and e not in blacklist:
                substring_matches.append(p)
    if substring_matches:
        # prefer exact equality
        for p in substring_matches:
            if p['name'].lower() == low or os.path.basename(p['exe']).lower() == low:
                return p['name']
        # otherwise return the shortest name match (more specific)
        substring_matches.sort(key=lambda x: len(x['name'] or ''))
        return substring_matches[0]['name']

    # 2) Token-match all words (useful for multi-word app names)
    tokens = [t for t in re.split(r'\s+', low) if t]
    if tokens:
        token_matches = []
        for p in procs:
            n = (p['name'] or '').lower()
            if all(t in n for t in tokens) and n not in blacklist:
                token_matches.append(p)
        if token_matches:
            token_matches.sort(key=lambda x: len(x['name'] or ''))
            return token_matches[0]['name']

    # 3) Fuzzy match on process names as a last resort, with higher cutoff
    names = [p['name'] for p in procs if p['name']]
    matches = difflib.get_close_matches(program_name, names, n=3, cutoff=max(threshold, 0.75))
    for m in matches:
        if m and m.lower() not in blacklist:
            return m
...
def close_program(program_name):
    ...
    try:
        # Find the closest matching running process (improved heuristics)
        ...
        # Safety blacklist to avoid terminating system-critical processes
        unsafe_names = set(['system', 'idle', 'services.exe', 'svchost.exe', 'wininit.exe', 'lsass.exe', 'csrss.exe'])

        if platform.system() == "Windows":
            # Collect candidate processes matching the chosen name (case-insensitive substring)
            candidates = []
            for proc in psutil.process_iter(['pid', 'name', 'exe']):
                try:
                    ...
                    if not pname:
                        continue
                    if closest_match.lower() == pname or closest_match.lower() in pname or (pexe and closest_match.lower() in os.path.basename(pexe).lower()):
                        # skip unsafe or system-level processes
                        if pname in unsafe_names or os.path.basename(pexe).lower() in unsafe_names:
                            continue
            ...
            if not candidates:
                # No safe candidates found; do not attempt to kill system processes
                return f"No safe process found matching '{closest_match}'."

            # Prefer exact name match, otherwise pick the most specific (shortest name)
            exact = None
            for p in candidates:
                try:
                    if (p.info.get('name') or '').lower() == closest_match.lower():
                        exact = p
                        break
                except Exception:
                    continue

            target_proc = exact if exact else sorted(candidates, key=lambda x: len(x.info.get('name') or ''))[0]

            try:
                pid = target_proc.info.get('pid')
                pname = target_proc.info.get('name')
                if pid in (0, 4) or (pname or '').lower() in unsafe_names:
                    return f"Refusing to terminate unsafe process: {pname} (pid={pid})"
                print(f"Terminating {pname} (PID {pid}) gracefully...")
                try:
                    target_proc.terminate()
                    target_proc.wait(timeout=5)
                    return f"Closed {pname} gracefully."
                except psutil.TimeoutExpired:
                    target_proc.kill()
                    return f"Force closed {pname}."
                except Exception as e:
                    return f"Error closing {pname}: {e}"
            except Exception as e:
                return f"Error selecting process to close: {e}"
            ...
            try:
                subprocess.run(["pkill", "-f", closest_match], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10)
                time.sleep(2)
                ...
                return f"Timeout closing {program_name}."
...
def chat_with_kevin(query, speak_to_kevin: bool = False, overrides: dict | None = None):
    ...
    try:
        ...
        # Subtle tone hint: use context awareness to gently influence LLM tone when available.
        try:
            if context_agent is not None:
                recent_texts = [m.get("content", "") for m in conversation_history[-6:]]
                combined = " ".join(recent_texts)
                mood = context_agent.detect_mood(combined)
                if mood:
                    hint = f"(Tone hint: respond slightly in a {mood} tone; keep changes subtle and concise.)\n\n"
                    # Prepend hint to the user's query so it acts as a light system prompt.
                    payload["text"] = hint + (payload.get("text") or "")
        except Exception:
            # silently ignore context hint failures
            pass
...
def main():
    ...
    try:
        def start_piper():
            ...
            candidates = [
                ...
                os.path.abspath(os.path.join(script_dir, '..', 'piper')),
            ]
            piper_dir = None
            for c in candidates:
    ...
    # Set a preferred Piper voice filename based on model
    if aiModel == "alice":
        current_tts_voice = 'en_US-hfc_female-medium.onnx'
    elif aiModel in ("virgil", "vrgl"):
        current_tts_voice = 'en_US-danny-low.onnx'
    else:
        current_tts_voice = 'ljspeech.onnx'
    ...
    context_aware = context_awareness.ContextAwareness()
    ...
    # Calendar reminders
    try:
        calendar_reminders.start_reminder_service(speak)
    except Exception as _cr:
        print(f"[Calendar] Reminder service failed to start: {_cr}")
    ...
    try:
        while flag == False:
            ...
            for idx, sub_query in enumerate(queries, start=1):
                ...
                # Handle different intent types (treat polite 'request' as command)
                ...
                # Add context awareness suggestion if mood detected
                try:
                    mood_suggestion = context_aware.analyze_conversation([msg for msg in conversation_history[-10:] if msg['role'] == 'user'])
                    if mood_suggestion.startswith("You seem"):
                        response += " " + mood_suggestion
                except Exception:
                    pass
                print("Assistant:", response)
                try:
