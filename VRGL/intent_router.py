...
def route_and_execute(user_text: str) -> Optional[str]:
    ...
    if fast:
        ...
        if action in COMMAND_SPECS:
            ...
            if action == "lock_computer" and not re.search(r"\block\b", user_text.lower()):
                ...
                p = _patterns_module()
                handler = getattr(p, COMMAND_SPECS[action]["handler"], None)
                if callable(handler):
                    args = fast.get("arguments", {}) or {}
                    try:
                        if action in ("search_web", "search_youtube"):
                            q = args.get("query") or _extract_search_query(user_text)
                            return handler(query=q) if q else handler()
                        if action == "doc_query":
                            q = args.get("question")
                            return handler(question=q) if q else handler()
                        if action == "start_timer":
                            dur = args.get("duration")
                            return handler() if not dur else handler(duration=str(dur))
                        if action == "set_volume":
                            if "delta" in args:
                                return handler(delta=float(args["delta"]))
                            if "volume" in args:
                                return handler(volume=float(args["volume"]))
                            return handler()
                        if action == "test_notification":
                            msg = args.get("message") if args else None
                            return handler(msg) if msg else handler()
                        return handler()
    ...
    try:
        if action in ("search_web", "search_youtube"):
            q = args.get("query") or _extract_search_query(user_text)
            return func(query=q) if q else func()
        if action == "doc_query":
            q = args.get("question")
            return func(question=q) if q else func()
        if action == "start_timer":
            duration = args.get("duration") or _extract_timer_duration_text(user_text)
            return func() if not duration else func(duration=str(duration))
        if action == "set_volume":
            vol_args = _extract_volume_args(user_text)
            if not vol_args:
            ...
            if "delta" in vol_args:
                return func(delta=float(vol_args["delta"]))
            if "volume" in vol_args:
                return func(volume=float(vol_args["volume"]))
            return func()
        if action == "test_notification":
            msg = args.get("message") if args else None
            return func(msg) if msg else func()
        # No-arg commands
        return func()
    except Exception as e:
        return f"Error executing {action}: {e}"
