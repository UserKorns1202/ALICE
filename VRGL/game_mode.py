...
import time, json, os, threading, difflib, random, ctypes, re
...
class GameModeContext:
    ...
    def add_or_update(self, name: str, sequence: List[str], hold: str = "left ctrl", delay: float = 0.14):
        self._stratagems[name.lower()] = Stratagem(name.lower(), [k.lower() for k in sequence], hold, delay)
    ...
    def _run_sequence(self, strat: Stratagem):
        ...
        _sendinput_sequence(strat.sequence, strat.hold, strat.delay)
    ...
    def interpret(self, spoken: str) -> Optional[Dict[str, str]]:
        ...
        if s.startswith("add stratagem "):
            ...
            if len(parts) >= 4:
                name = parts[2]
                seq = [k for k in parts[3:] if k in VK_MAP]
                if seq:
                    self.add_or_update(name, seq)
                    return {"type": "game_mode_add", "spoken": f"Added stratagem {name}."}
            return {"type": "game_mode_error", "spoken": "Could not parse stratagem addition."}
